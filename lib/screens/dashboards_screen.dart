...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      _totalAchats = await _getTotal(
        'purchases',
        'purchase_date',
        periodStart,
        periodEnd,
      );
      _totalVentes = await _getTotal(
        'sales',
        'sale_date',
        periodStart,
        periodEnd,
      );
      _totalDepenses = await _getTotal(
        'expenses',
        'created_at',
        periodStart,
        periodEnd,
  ...
  Future<void> _getProductStats(DateTime? start, DateTime? end) async {
    ...
    _bestSoldProducts = sorted
        .take(5)
        .map((e) => {'name': e.key, 'quantity': e.value})
        .toList();
    _leastSoldProducts = sorted.reversed
        .take(5)
        .map((e) => {'name': e.key, 'quantity': e.value})
        .toList();
  }
                  ...
                  children: [
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        ...
                        children: ['Semaine', 'Mois', 'Année', 'Toutes'].map((
                          period,
                        ) {
            ...
            ...products.map(
              (p) => ListTile(
                ...
                leading: CircleAvatar(
                  ...
                  child: Text(
                    "${(p['quantity'] as double).toInt()}",
                    style: TextStyle(color: color),
                  ),
                ),
                title: Text(p['name'] as String),
                trailing: Text(
                  "${(p['quantity'] as double).toInt()} vendus",
