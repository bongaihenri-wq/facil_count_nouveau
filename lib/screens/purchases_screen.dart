...
class _PurchasesScreenState extends State<PurchasesScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final productsRes = await supabase.from('products').select().order('name');
      ...
      _totalMoisActuel = await _getTotal('purchases', 'purchase_date', moisActuelStart, moisActuelEnd);
      _totalMoisPrecedent = await _getTotal('purchases', 'purchase_date', moisPrecedentStart, moisPrecedentEnd);
      _difference = _totalMoisActuel - _totalMoisPrecedent;

      if (mounted) {
        setState(() {
          _purchases = List<Map<String, dynamic>>.from(purchasesRes);
  ...
  List<Map<String, dynamic>> get filteredPurchases {
    ...
    if (productFilter.isNotEmpty) {
      ...
      list = list.where((p) => (p['products']?['name'] as String?)?.toLowerCase().contains(q) ?? false).toList();
    }
    ...
    if (exactQuantity != null) {
      list = list.where((p) => (p['quantity'] as int?) == exactQuantity).toList();
    }
  ...
  num getTotalPurchases() => filteredPurchases.fold<num>(0, (sum, item) => sum + (item['amount'] as num? ?? 0));

  @override
  Widget build(BuildContext context) {
    final total = getTotalPurchases();
    final displayedList = filteredPurchases;
        ...
        actions: [
          IconButton(
            icon: Container(
              ...
              child: Icon(Icons.filter_list, color: Colors.blue.shade700, size: 22),
            ),
              ...
              children: [
                    ...
                    child: Card(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text('Total achats', style: TextStyle(fontSize: isSmall ? 15 : 17)),
                            FittedBox(
                ...
                Expanded(
                  child: _selectedTab == 'Liste'
                      ? _buildPurchasesList(displayedList)
  ...
  Widget _buildPurchasesList(List<Map<String, dynamic>> data) {
    if (data.isEmpty) {
          ...
          builder: (context, setModalState) {
            return Padding(
              ...
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Filtrer les achats', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  TextField(
                    decoration: const InputDecoration(labelText: 'Nom du produit (contient)', border: OutlineInputBorder()),
                    onChanged: (val) {
                  ...
                  const SizedBox(height: 12),
                  TextField(
                    decoration: const InputDecoration(labelText: 'Quantité exacte', border: OutlineInputBorder()),
                ...
                child: Column(
                  ...
                  children: [
                    FutureBuilder<List<Map<String, dynamic>>>(
                      future: supabase.from('products').select('id, name, stock').order('name'),
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (selectedProductId == null ||
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Produit, quantité et montant obligatoires')),
                    );
                  }
                  return;
                }
                ...
                try {
                  ...
                  await supabase.from('products').update({'stock': currentStock + quantity}).eq('id', selectedProductId!);
  ...
  void _showEditPurchaseDialog(Map<String, dynamic> purchase) {
    ...
    final invoiceCtrl = TextEditingController(text: purchase['invoice_number'] ?? '');
    final quantityCtrl = TextEditingController(text: purchase['quantity'].toString());
    ...
    DateTime purchaseDate = DateTime.tryParse(purchase['purchase_date'] ?? '') ?? DateTime.now();
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  if (selectedProductId != oldProductId) {
                    // restore to old product
                    final oldProductRes = await supabase.from('products').select('stock').eq('id', oldProductId).single();
                    final oldStock = (oldProductRes['stock'] as int?) ?? 0;
                    await supabase.from('products').update({'stock': oldStock - oldQuantity}).eq('id', oldProductId);

                    // add to new product
                    final newProductRes = await supabase.from('products').select('stock').eq('id', selectedProductId!).single();
                    final newStock = (newProductRes['stock'] as int?) ?? 0;
                    await supabase.from('products').update({'stock': newStock + quantity}).eq('id', selectedProductId!);
                  } else {
                    // same product, adjust difference
                    final productRes = await supabase.from('products').select('stock').eq('id', selectedProductId!).single();
                    final currentStock = (productRes['stock'] as int?) ?? 0;
                    await supabase.from('products').update({'stock': currentStock + (quantity - oldQuantity)}).eq('id', selectedProductId!);
                  }
  ...
  Future<void> _deletePurchase(Map<String, dynamic> purchase) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Voulez-vous supprimer cet achat du ${purchase['purchase_date']?.substring(0, 10) ?? 'date inconnue'} ?'),
        actions: [
          TextButton(
    ...
    try {
      ...
      final productRes = await supabase.from('products').select('stock').eq('id', productId).single();
      final currentStock = (productRes['stock'] as int?) ?? 0;
      await supabase.from('products').update({'stock': currentStock - quantity}).eq('id', productId);

      if (mounted) {
        await _loadData();
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Achat supprimé et stock ajusté')));
      }
    } catch (e) {
      if (mounted) {
